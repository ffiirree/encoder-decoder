...
__all__ = ['UNet3Plus2DecodersShortcuts']
...
class UNet3Plus2DecodersShortcuts(nn.Module):
    def __init__(self, n_channels, n_classes, n_ex_channels, filters = [64, 128, 256, 512, 1024]):
        ...
        self.decode_conv1 = DoubleConv2d(filters[3] * 3 + filters[0] + filters[1] + filters[2], filters[3])
        self.up2 = nn.ConvTranspose2d(filters[3], filters[2], kernel_size=4, stride=2, padding=1)
        self.decode_conv2 = DoubleConv2d(filters[2] * 3 + filters[0] + filters[1] + filters[4], filters[2])
        self.up3 = nn.ConvTranspose2d(filters[2], filters[1], kernel_size=4, stride=2, padding=1)
        self.decode_conv3 = DoubleConv2d(filters[1] * 3 + filters[0] + filters[4] + filters[3], filters[1])
        self.up4 = nn.ConvTranspose2d(filters[1], filters[0], kernel_size=4, stride=2, padding=1)
        self.decode_conv4 = DoubleConv2d(filters[0] * 3 + filters[4] + filters[3] + filters[2], filters[0])
...
if __name__ == '__main__':
    ...
    model = UNet3Plus2DecodersShortcuts(3, 1, n_ex_channels=3, filters=[32, 64, 128, 256, 512])
